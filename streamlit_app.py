...
st.set_page_config(page_title="한화 직링 생성기", layout="centered")
...
def get_ticketlink_server_time_with_ms():
    try:
        response = requests.get("https://time.navyism.com/?host=www.ticketlink.co.kr")
        ...
        return "❌ 서버 시간 불러오기 실패"

st.subheader("🕒 티켓링크 서버 시간 (실시간)")
placeholder = st.empty()
for _ in range(5):
    placeholder.code(get_ticketlink_server_time_with_ms())
    time.sleep(1)
...
if st.button("직링 생성"):
    ...
    try:
        ...
        if not schedules:
            ...
            match_time = datetime.fromtimestamp(schedule['scheduleDate'] / 1000, tz=timezone(timedelta(hours=9))).strftime("%Y년 %m월 %d일 %H:%M")
            link = f"https://www.ticketlink.co.kr/reserve/product/{product_id}?scheduleId={schedule_id}"
            st.success(f"🔗 직링: {link}")
            st.info(f"- 🏟️ 경기: {home_team} vs {away_team}  \n- 🎯 구간: {match_title}  \n- 🕒 시작 시간: {match_time}")
    except Exception as e:
        st.error(f"❌ 오류 발생: {e}")
